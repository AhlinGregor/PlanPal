...
export function EditEventComponent({
	...
	useEffect(() => {
		if (event && visible) {
			...
			// Parse the event date from ISO string
			const eventDate = new Date(event.date + 'T00:00:00');

			// Set start datetime from startHour
			...
			// Set end datetime from endHour
			const endHours = Math.floor(event.endHour);
			const endMinutes = Math.round((event.endHour - endHours) * 60);
